...
export class ProfileData {
  public userProfileRef: any; // We'll use this to create a database reference to the userProfile node.
  ...
  constructor() {
    ...
    */
    ...
    this.userProfileRef = firebase.database().ref('/userData/userProfile');
  ...
  */
  getUserProfile(): any {
    return this.userProfileRef.child(this.currentUser.uid);
  }


  getUser(key): any {
    return this.userProfileRef.child(key);
  }
  ...
  */
  updateName(name: string, surname: string): any {
    return this.userProfileRef.child(this.currentUser.uid).update({
      name: name,
      surname: surname,
    });
  }
  ...
  */
  updateEmail(newEmail: string): any {
    this.currentUser.updateEmail(newEmail).then(() => {
      this.userProfileRef.child(this.currentUser.uid).update({
        email: newEmail
      });
    }, (error) => {
      console.log(error);
